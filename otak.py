...
def staff_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not u or u.role not in ['admin', 'moderator']:
            flash("Akses Khusus Admin/Moderator!")
            return redirect('/')
        return f(*args, **kwargs)
    return decorated_function
        ...
        <div class="d-flex align-items-center animate__animated animate__fadeInRight">
            {% if session.user_id %}
                {% if session.role in ['admin', 'moderator'] %}
                    <a href="/admin" class="btn btn-outline-warning btn-sm me-3"><i class="bi bi-shield-lock"></i> Panel Kendali</a>
                {% endif %}
    ...
    <div class="container mt-4 mb-5" data-aos="zoom-in">
        <div class="glass-card mx-auto" style="max-width:550px; border: 1px solid var(--gold);">
            ...
            <form method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-6"><label class="small text-gold">Username</label><input name="u" class="form-control" required></div>
                    <div class="col-6"><label class="small text-gold">Password</label><input type="password" name="p" class="form-control" required></div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-6"><label class="small text-gold">Kelas</label><input name="kls" class="form-control" required></div>
                </div>
...
def profil():
    ...
    <div class="container mt-5" data-aos="fade-up"><div class="row justify-content-center"><div class="col-md-6"><div class="glass-card">
        ...
        {% if u.role in ['admin', 'moderator'] %}
            <a href="/admin" class="btn btn-warning btn-sm w-100 mb-2">PANEL KENDALI</a>
        {% endif %}
        <a href="/edit-profil" class="btn btn-outline-info btn-sm w-100 mb-2">EDIT PROFIL</a>
...
def edit_profil():
    ...
    return render_template_string(UI_CORE_HEADER + """
    <div class="container mt-4"><div class="glass-card mx-auto shadow" style="max-width:500px;">
        <h3 class="text-gold fw-bold text-center">EDIT PROFIL</h3>
        <form method="POST">
            <input name="fn" class="form-control mb-3" value="{{ u.full_name }}" required>
            <input name="gm" class="form-control mb-3" value="{{ u.gmail }}" required>
            <input name="nis" class="form-control mb-3" value="{{ u.nis }}" required>
            <input name="kls" class="form-control mb-3" value="{{ u.kelas }}" required>
...
def struktur():
    ...
    <div class="container mt-5">
        <h3 class="text-gold fw-bold mb-4">STRUKTUR PIMPINAN</h3>
        {% if session.role in ['admin', 'moderator'] %}
        <div class="glass-card mb-4">
            <h6 class="text-gold">Tambah Pengurus</h6>
            <form method="POST" class="row g-2">
                <div class="col-md-4"><input name="jabatan" class="form-control" placeholder="Jabatan"></div>
                <div class="col-md-4"><input name="nama" class="form-control" placeholder="Nama"></div>
        ...
        {% endif %}
        <div class="glass-card"><table class="table table-dark">
            ...
            {% if session.role == 'admin' %}<td><a href="/hapus/struktur/{{ p.id }}" class="text-danger">Hapus</a></td>{% endif %}</tr>{% endfor %}</tbody>
        </table></div>
...
def absen():
    ...
    <div class="container mt-5"><div class="row">
        <div class="col-md-4 mb-4"><div class="glass-card text-center">
            <h4 class="text-gold">PRESENSI</h4>
            <form method="POST"><button type="submit" class="btn-gold py-4 fs-3">HADIR</button></form>
        </div></div>
        <div class="col-md-8"><div class="glass-card">
            <table class="table table-dark small">
                <thead><tr><th>Kader</th><th>Waktu</th></tr></thead>
                <tbody>{% for l in logs %}<tr><td>{{ l.nama_kader }}</td><td>{{ l.waktu_hadir.strftime('%H:%M - %d/%m') }}</td></tr>{% endfor %}</tbody>
            </table>
        </div></div>
...
def kas():
    ...
    <div class="container mt-4">
        <div class="glass-card text-center mb-4">
            <small class="text-secondary">SALDO KAS OTOMATIS</small>
            <h1 class="text-gold fw-bold">Rp {{ "{:,}".format(saldo) }}</h1>
        </div>
        <div class="row">
            {% if session.role in ['admin', 'moderator'] %}
            <div class="col-md-4 mb-3"><div class="glass-card">
                <form method="POST">
                    <select name="t" class="form-select mb-3"><option value="masuk">Masuk</option><option value="keluar">Keluar</option></select>
            ...
            <div class="col-md-{{ '8' if session.role in ['admin', 'moderator'] else '12' }}">
                <div class="glass-card"><table class="table table-dark">
                    <thead><tr><th>Ket</th><th>Jumlah</th></tr></thead>
                    <tbody>{% for d in data %}<tr><td>{{ d.keterangan }}</td><td class="{{ 'text-success' if d.tipe == 'masuk' else 'text-danger' }}">Rp {{ "{:,}".format(d.jumlah) }}</td></tr>{% endfor %}</tbody>
                </table></div>
            </div>
        </div>
...
def agenda():
    ...
    return render_template_string(UI_CORE_HEADER + """
    <div class="container mt-5">
        <h3 class="text-gold fw-bold text-center mb-4">AGENDA KEGIATAN</h3>
        {% if session.role in ['admin', 'moderator'] %}
        <div class="glass-card mb-4">
            <form method="POST" class="row g-2">
                <div class="col-md-4"><input name="judul" class="form-control" placeholder="Kegiatan"></div>
                <div class="col-md-3"><input name="waktu" class="form-control" placeholder="Waktu"></div>
                <div class="col-md-3"><input name="lokasi" class="form-control" placeholder="Lokasi"></div>
                <div class="col-md-2"><button type="submit" class="btn-gold">BUAT</button></div>
            </form>
        </div>
        {% endif %}
        <div class="row">{% for a in semua_agenda %}<div class="col-md-4 mb-3"><div class="glass-card border-start border-gold">
...
# --- ROUTES LAPORAN & HAPUS (KHUSUS ADMIN) ---
...
def lapor():
    if request.method == 'POST':
        ...
        return redirect('/')
    return render_template_string(UI_CORE_HEADER + """
    <div class="container mt-5"><div class="glass-card mx-auto shadow" style="max-width:500px; border: 1px solid red;">
        <h3 class="text-danger">LAPOR BUG</h3>
        <form method="POST"><textarea name="pesan" class="form-control mb-3" rows="5" required></textarea><button type="submit" class="btn btn-danger w-100">KIRIM</button></form>
    </div></div>""" + UI_CORE_FOOTER)
...
@staff_only
def admin_panel():
    u_aktif = User.query.get(session['user_id'])
    ...
    return render_template_string(UI_CORE_HEADER + """
    <div class="container mt-5">
        <h2 class="text-gold fw-bold mb-4">PANEL KONTROL ({{ u_aktif.role.upper() }})</h2>
        <div class="row mb-4">
            <div class="col-md-6"><div class="glass-card">SALDO KAS: <span class="text-gold">Rp {{ "{:,}".format(total_kas) }}</span></div></div>
            <div class="col-md-6"><div class="glass-card">TOTAL KADER: {{ users|length }}</div></div>
        </div>

        <div class="glass-card">
            <h5 class="text-gold">Manajemen Akun</h5>
            <div class="table-responsive"><table class="table table-dark small">
                ...
                <tbody>{% for u in users %}<tr>
                    ...
                    <td>
                        {% if u_aktif.role == 'admin' and u.role == 'member' %}
                        <a href="/promote/{{ u.id }}" class="btn btn-sm btn-outline-warning">Naik Moderator</a>
                        {% endif %}
                        {% if u_aktif.role == 'admin' and u.username != 'dafitrah' %}
                        <a href="/hapus/user/{{ u.id }}" class="text-danger ms-2">Hapus</a>
                        {% endif %}
                    </td>
                </tr>{% endfor %}</tbody>
            </table></div>
        ...
        <div class="glass-card mt-4">
            <h5 class="text-danger">Laporan Bug</h5>
            {% for l in laporan %}
                <div class="p-2 border-bottom border-secondary small">{{ l.nama_pelapor }}: {{ l.pesan }}</div>
            {% endfor %}
        </div>
    </div>""" + UI_CORE_FOOTER, users=users, total_kas=total_kas, laporan=laporan, u_aktif=u_aktif)
...
def promote(id):
    ...
    u = User.query.get(id)
    if u: u.role = 'moderator'; db.session.commit()
...
def hapus_struktur(id):
    if session.get('role') != 'admin': return redirect('/')
    s = Struktur.query.get(id); db.session.delete(s); db.session.commit()
    return redirect('/struktur')

# --- KTA & PIAGAM (FITUR ASLI DAFI) ---
    ...
    <div class="container mt-5 text-center">
        ...
        <div class="kta-card p-3 shadow-lg">
            <div class="d-flex justify-content-between text-start">
                <div><small class="text-gold">IPM SMKM 1 TGR</small><br><small>NIS: {{ u.nis }}</small></div>
                <i class="bi bi-qr-code text-white fs-3"></i>
            </div>
            <div class="text-start mt-3">
                <h5 class="text-white mb-0">{{ u.full_name }}</h5>
...
def piagam():
    ...
    <html><body style="background:#0b0d11; color:white; text-align:center; padding:50px;">
        <div style="border:10px solid #d4af37; padding:50px; background:#1a1c20;">
            <h1 style="color:#d4af37;">PIAGAM PENGHARGAAN</h1>
            <p>Diberikan kepada:</p>
            <h2 style="font-size:3rem;">{{ u.full_name }}</h2>
            <p>Atas dedikasinya dalam Ikatan Pelajar Muhammadiyah.</p>
            <br><a href="/profil" style="color:#d4af37;">Kembali ke Profil</a>
        </div>
    </body></html>""", u=u)
